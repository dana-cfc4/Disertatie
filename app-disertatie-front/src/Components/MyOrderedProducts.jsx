  ...
  const getCurrentFavorites = () => {
    ...
    if (auth) {
      orders.filter(order => order.idUtilizator === auth?._id).map(order => {
        order.cosCumparaturi.produse.map(produ => {
          orderedProducts = [...new Set([...orderedProducts, produ.idProdus])];
        })
      })
      const filteredProducts = products.filter(product => {
        return orderedProducts.includes(product._id)
      })
          ...
          {getCurrentFavorites().length > 0 ? (
            <>
              {getCurrentFavorites().map((favorit) => {
                return (
                  <div key={favorit._id} style={{ textAlign: "left" }}>
                    <Box
                      sx={{
                        bgcolor: "#ffffff",
                        height: "fit-content",
